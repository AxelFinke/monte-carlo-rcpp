...
#$ -o $HOME/Scratch/output/blockedSmoothing/linearGaussianHmm/log_par.$JOB_ID.txt -j y

# Run the application
module unload compilers
module unload mpi
module load r/recommended
R --no-save < $HOME/blockedSmoothing/examples/linearGaussianHmm/block_par.r > $HOME/Scratch/output/blockedSmoothing/linearGaussianHmm/r_output_par.$JOB_ID.$SGE_TASK_ID

# Preferably, tar-up (archive) all output files onto the shared scratch area
# this will include the R_output file above.
tar zcvf $HOME/Scratch/output/blockedSmoothing/linearGaussianHmm/files_from_job_par.$JOB_ID.$SGE_TASK_ID.tgz $TMPDIR
