...
N_LEVELS     <- rep(N_LEVELS_AUX, times=1) ###
# Maximum number of levels in the step functions for the productivity parameter
# used by some models:

N_AGE_GROUPS_AUX <- c(4)
N_AGE_GROUPS     <- rep(N_AGE_GROUPS_AUX, each=length(MODELS)) 
# Maximum number of age groups

N_CONFIGS  <- length(MODELS) # number of different model/algorithm configurations

CESS_TARGET              <- rep(0.99, times=N_CONFIGS) # target conditional effective sample size used for adaptive tempering
...
LOWER                    <- rep(3, times=N_CONFIGS) # type of algorithm for updating the count-data likelihood (0: pseudo-marginal; 2: MCWM; 3: exact evaluation (of approximate linear-Gaussian likelihood via the Kalman filter))
...
for (ii in 1:N_CONFIGS) {
  if (MODELS[ii] %in% c(3:4)) {
    MODEL_NAMES <- c(MODEL_NAMES, paste(MODEL_TYPE_NAMES[MODELS[ii]+1], " (", N_LEVELS[ii], " different regimes)", sep=''))
